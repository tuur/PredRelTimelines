...
class Text(object):
	...
	def __init__(self, text, span_annotations={}, span_pair_annotations={}, id=0, lowercase=True, pos=False, transitive_closure=[], conflate_digits=True, measure_speed=False):
		if measure_speed:
			t0 = time.time()
	...
	def write_to_brat(self, directory, span_label_filter=None, span_pair_label_filter=None):
		...
		relation_index= 1
		span_to_brat_index = {}
		#for key, value in sorted(self.reverse_span_annotations.items(), key=lambda x: x[0][0]):
		#	print(key, value)
		#print('-------')
		#for key, value in sorted(self.span_annotations.items(), key=lambda x:x[0]):
		#	print(key, value)
				...
				if not span_pair_label_filter or re.search(span_pair_label_filter, label):
					for span1, span2 in self.span_pair_annotations[label]:
						...
						if not span2 == (0,0):
							...
							relation_index += 1
#			pass
#			print(self.span_annotations.keys()[:10])
